<template>
    <div class="bet">
        <div class="betArea" :class="{ open: isShowBetArea, close: !isShowBetArea }" :style="{ height: keyboardShow ? '315px' : '' }">
            <div v-if="filterData.length" class="flex items-center justify-between mt-2 mx-3">
                <div
                    class="btn flex justify-center items-center"
                    v-for="(item, index) in itemData.play"
                    :key="index"
                    @click="betDetail(itemData, item)"
                    :class="{ activeBtn: isActive === item.playCode }"
                >
                    投
                    <span class="mx-1" :style="{ color: filterColor(item) }">
                        {{ item.playName }}
                    </span>
                    <span>{{ item.odds }}</span>
                </div>
        ...
        <!-- 投注确认弹窗 -->
        <van-dialog
            v-model:show="betComfirmDialogShow"
            class="betComfirmDialog"
            title="投注确认"
            show-cancel-button
            @cancel="betComfirmDialogCancel"
            @confirm="betComfirmDialogConfirm(isBet, itemData)"
        >
            <div class="my-2 mx-3">
                <div class="currentLottery">{{ isOneLottery ? currentLottery.label : itemData.lotteryName }}</div>
                <div class="period">{{ $t('betting.period', { No: issueDisplay(isOneLottery ? openInfo.nextLotteryNum : itemData.nextLotteryNum) }) }}</div>
            </div>
...
const isActive = computed(() => commonStore.isActive);
...
const emit = defineEmits(['update:betDetail', 'update:chase', 'update:isShowBetArea', 'update:reset']);

let timer = null; // 查余额定时器

const filterColor = item => {
    const blueColor = ['单', '小', '虎'];
    const redColor = ['双', '大', '龙'];
...
const betDetail = (item, odd) => {
    emit('update:betDetail', item, odd);
    commonStore.setIsActive(odd.playCode);
};
...
// 立即投注确认提交
const betComfirmDialogConfirm = (itemData, isBet) => {
    ...
    trigger.value = 1;
    const data = { betList, lotteryCode: isBet.play[0].lotteryCode, lotteryNum: isBet.nextLotteryNum };
    lotteryBet(data)
        .then(res => {
...
const cartPopupConfirm = () => {
    ...
    if (trigger.value) return;
    trigger.value = 1;
    let data = { betList: cartData.value, lotteryCode: currentLottery.value.code, lotteryNum: itemData.nextLotteryNum };
    lotteryBet(data)
        .then(res => {
            showToast(t('tip.Successful')); //下注成功
            reset();
...
const showChase = () => {
    if (!betAmount.value) {
        ...
        return;
    } else if (Object.keys(props.isBet).length === 0) {
        showToast('请选择或者输入投注金额');
        return;
    }
...
const chasePopupConfirm = () => {
    ...
    chaseNumberChase({
        lotteryCode: haoluChaseList.value.play[0].lotteryCode,
...
watch(
    ...
    newValue => {
        ...
        if (storedData || !props.isOneLottery) {

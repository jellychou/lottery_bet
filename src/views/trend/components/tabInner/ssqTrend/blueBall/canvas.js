...
export const drawTable = (canvas, newData, type = 1, issueTitle, trendTitle) => {
    ...
    const columns = 16;
    const columnWidth = 99.8;
    const columnHeignt = 99.8;
    ...
    const drawColumn = ({ text, width, height, x, y, bgColor = '#fff', fontSize = '56px', heighlight = false, fieldIndex, color = textColor, fontWeight = '400' }) => {
        // Draw column
        ctx.beginPath();
    ...
    const drawRedLine = () => {
        ...
        Object.keys(winningPosition).forEach((key, index, keys) => {
            const current = winningPosition[keys[index]];
            const next = winningPosition[keys[index + 1]];
        ...
        Object.values(winningPosition).forEach(position => {
            const displayText = Math.abs(position.text) <= 9 ? `0${Math.abs(position.text)}` : Math.abs(position.text);
            if (position) {
                // 再次确认position存在

        ...
        <div class="display h-full overflow-hidden">
            <div v-if="rD.rodio" class="flex items-center mt-2 px-2.5">
                <van-tabs v-model:active="rodioIndex" class="radio" line-height="0" :ellipsis="false">
                    <van-tab v-for="(it, i) in rD.rodio" :key="i" :title="t(it.name)"></van-tab>
                </van-tabs>
                <div class="radio_separate flex-shrink-0">
            ...
            <div class="main_play" :style="{ height: rD.checkbox ? 'calc(100dvh - 446px)' : 'calc(100dvh - 371px)' }">
...
const rD = computed(() => {
    const agr = rodioIndex.value + hleper.value && play.value && lotteryItems.value && currentLottery.value.parentCode === 'c11x5'; // 直接使用 lotteryData 作为依赖项
    let result = {};
    if (agr) {
        try {
...
const CalcLen = Chosedata => {
    ...
    switch (play.value) {
        case 'kj':
            ...
            if (checkedArr.length) {
                checkedArr.forEach(_ => {
                    Chosedata.forEach(i => finalData.push(storeData.value[_.value].find(_i => _i.playCode === i.playCode.replace('1', _.value))));
                });
            } else {
        ...
        case 'zux':
            ...
            break;
        case 'rx':
            handleZx(Chosedata, rodioIndex.value + 1, finalData);
            break;
        default:
            finalData = Chosedata;
            break;
...
<!-- 老板需求 快捷、第一球、第二球、第...大小单双由长方形改为圆形投注按钮-->

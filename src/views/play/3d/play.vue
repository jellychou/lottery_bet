...
const rD = computed(() => {
    const agr = rodioIndex.value + hleper.value && play.value && lotteryItems.value && currentLottery.value.parentCode === '3d'; // 直接使用 lotteryData 作为依赖项
    let result = {};
    if (agr) {
        try {
...
const CalcLen = Chosedata => {
    ...
    switch (play.value) {
        case 'kj':
            ...
            break;
        case 'zxs':
            handleZx(Chosedata, rodioIndex.value, finalData, 1);
            break;
        case 'zxl':
            handleZx(Chosedata, rodioIndex.value, finalData);
            break;
        default:
            finalData = Chosedata;
            break;

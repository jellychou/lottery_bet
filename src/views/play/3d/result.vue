<template>
    <div class="result">
        ...
        <div class="flex" v-if="!isLotteryDrawing && lotteryResult">
            <LotteryBall v-for="(num, inx) of gameResult" :key="inx" :i="num" :blur="inx" :delay="inx + 1" :ball="11" :percent="-9.09" />
        </div>
...
<script setup>
import { getCurrentInstance, onMounted, provide, ref, nextTick, reactive, computed, onUnmounted } from 'vue';
...
const gameResult = computed(() => {
    ...
    return lotteryResult.value && arr.length === 3 ? arr : [0, 0, 0];
});
</script>

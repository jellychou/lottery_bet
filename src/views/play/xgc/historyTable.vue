...
<template>
    <el-table :data="data" stripe height="350" border style="width: 100%" :highlight-current-row="false">
        <el-table-column label="期号" width="110" align="center">
        ...
        <el-table-column label="开奖号码" align="center">
            <template v-slot:default="scope">
                <div class="flex">
                    <div class="ball mr-2" v-for="item in scope.row.result.split(',').slice(0, 6)">{{ item }}</div>
                </div>
            </template>
        </el-table-column>
        <el-table-column width="70" label="特码" align="center">
            <template v-slot:default="scope">
                <div class="flex">
                    <div class="ball greenBall mr-2" v-for="item in scope.row.result.split(',').slice(6, 7)">{{ item }}</div>
                    <div v-for="item in scope.row.resultProperty.split(',').slice(6, 7)">{{ item }}</div>
                </div>
            </template>
        </el-table-column>
    </el-table>

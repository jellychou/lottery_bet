        ...
        <div class="display h-full">
            ...
            <div class="main_play">
                ...
                <ul v-for="(item, i) in rD.sort || (rD.sorts && rD.sorts[rodioIndex]) || []" :key="i" :class="[{ square: item.square, ball: item.ball, 'pt-3': i === 0 }]">
                    <div class="sort_title px-2.5 pb-2">
...
const rD = computed(() => {
    const agr = rodioIndex.value + hleper.value && play.value && lotteryItems.value && currentLottery.value.parentCode === 'ssc'; // 直接使用 lotteryData 作为依赖项
    let result = {};
    if (agr) {
        try {
    ...
    switch (play.value) {
        case 'kj':
            ...
            if (checkedArr.length) {
                checkedArr.forEach(_ => Chosedata.forEach(i => finalData.push(storeData.value[_.value].find(_i => _i.playCode === i.playCode.replace('w', _.id)))));
            } else {
                finalData.err = t('rules.bet_rule');
            } // '请选择顶部投注类型'
            break;
        default:
            finalData = Chosedata;
